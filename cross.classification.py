...
if percent == 100:
    outfile = features + '-' + classifier + '-' + testcategory + '-' + sampleid
else:
    outfile = features + '-' + classifier + '-' + testcategory + '-' + sampleid + '-' + str(percent) + 'p'
...
if 'phrases' in features:
    for category in categories:
        ...
        if category == testcategory:
            dataset = 'test'
        for gt in range(1, 6):
            with open('phrases-' + sampleid + '/data-' + category + '-' + str(gt) + '.jsonl', 'r') as fh:
                for line in fh:
                    idcounter += 1
                    review = json.loads(line)
                    if dataset == 'test' or countin < percent:
                        if dataset == 'train':
                            countin += 1
                    ...
                    elif countout < 100 - percent:
                        countout += 1

                    if countin + countout == 100:
                        countin = 0
                        countout = 0
...
if 'posneg' in features:
    for category in categories:
        ...
        if category == testcategory:
            dataset = 'test'
        for gt in range(1, 6):
            with open('posneg-' + sampleid + '/data-' + category + '-' + str(gt) + '.jsonl', 'r') as fh:
                for line in fh:
                    idcounter += 1
                    review = json.loads(line)
                    if dataset == 'test' or countin < percent:
...
countin = 0
countout = 0
if features != 'phrases' and features != 'posneg':
    for category in categories:
        dataset = 'train'
        if category == testcategory:
            dataset = 'test'
        for gt in range(1, 6):
            with open('review-data-sample-' + sampleid + '/data-' + category + '-' + str(gt) + '.jsonl', 'r') as fh:
                for line in fh:
                    idcounter += 1
                    review = json.loads(line)
                    if dataset == 'test' or countin < percent:
...
if features == 'unigrams':
    ngram_vectorizer = CountVectorizer(ngram_range=(1, 1), token_pattern=r'\b\w+\b', min_df=250)
